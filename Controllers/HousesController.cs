...
public class HousesController : ControllerBase
{
  ...
  public async Task<ActionResult<House>> UpdateHouse(int houseId, [FromBody] House updateHouseData)
  {
    try
    {
      Account userInfo = await _auth0Provider.GetUserInfoAsync<Account>(HttpContext);
      House house = _housesService.UpdateHouse(houseId, updateHouseData, userInfo);
      return Ok(house);
    }
    catch (Exception exception)
    {
